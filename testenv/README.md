...
- registry.k8s.io is geo-blocked from the VMs (403; quay.io and docker.io are
  reachable, so those images come online). The fix for any (re)build is
  `download_localhost: true` + `download_run_once: true` in
  inventory/group_vars/k8s_cluster/k8s-cluster.yml: kubespray pulls on the
  ansible controller (host docker has the proxy; images are already cached
  there) and uploads to the nodes itself after containerd is installed — no
