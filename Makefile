...
test: manifests generate fmt vet setup-envtest ## Run tests.
	KUBEBUILDER_ASSETS="$(shell "$(ENVTEST)" use $(ENVTEST_K8S_VERSION) --bin-dir "$(abspath $(LOCALBIN))" -p path)" go test $$(go list ./... | grep -v /e2e) -coverprofile cover.out
...
KIND_CLUSTER ?= pbs-operator-test-e2e
...
docker-buildx: ## Build and push docker image for the manager for cross-platform support
	...
	- $(CONTAINER_TOOL) buildx create --name pbs-operator-builder
	$(CONTAINER_TOOL) buildx use pbs-operator-builder
	...
	- $(CONTAINER_TOOL) buildx rm pbs-operator-builder
	rm Dockerfile.cross
